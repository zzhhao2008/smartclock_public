function getIconPath(iconname, size)
    local path = "/luadb/icon_" .. iconname .. "-" .. size .. ".png"
...
function remoteCfgdeal(data,savesyscfNow)
    log.info("remoteCfg Dealing", json.encode(data))
    if data['highlighteddays'] ~= nil then
        highlighteddays = data.highlighteddays
        dbPut("highlighteddays", highlighteddays)
    end
    if data['filesToDownload'] ~= nil then
        sysConfig.filesToDownload.files = data.filesToDownload
        if savesyscfNow~=nil then
            dbPut("sysConfig", sysConfig)
        end
    end
    ...
    if data['morewifi'] ~= nil then
        morewifi  = data.morewifi
    end
end
